...
use PhpOffice\PhpSpreadsheet\Spreadsheet; // Export Excel
...
class BarangController extends Controller
{
                ...
                foreach ($data as $baris => $value) {
                    if($baris > 1){ // baris ke 1 adalah header, maka lewati
                        $insert[] = [
                            ...
                            'harga_beli' => $value['D'],
                            'harga_jual' => $value['E'],
                            'supplier_id' => 1, // Default supplier (karena di excel tidak ada kolom supplier)
                            'created_at' => now(),
                        ];
                    }
                }
    ...
    public function export_excel()
    {
        ...
        $barang = BarangModel::select('kategori_id', 'barang_kode', 'barang_nama', 'harga_beli', 'harga_jual')
            // ->orderBy('barang_kode') // <--- GANTI JADI INI (Urut Kode)
            ->orderBy('kategori_id')
            ->with('kategori')
            ->get();
